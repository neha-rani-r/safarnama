...
OUTPUT: plain prose only. No headers. No bullet points. No hashtags.`,
  ...
  const downloadCard = async () => {
    ...
    try {
      ...
      if (!blob) { showToast('Could not capture card'); return; }
  ...
  const shareCard = async () => {
    ...
    try {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({ files: [file], title: `Safarnama — ${primaryLocation}` });
      } else {
        // Fallback: download
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
            ...
            <div>
              ...
              <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                <button
                  ...
                  disabled={capturing}
                  style={{ flex: 1, padding: '13px', background: 'var(--surface)', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: 10, cursor: capturing ? 'default' : 'pointer', fontFamily: 'var(--sans)', fontSize: 13, fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: capturing ? 0.5 : 1 }}
                >
                ...
                <button
                  onClick={shareCard}
                  disabled={capturing}
                  style={{ flex: 1, padding: '13px', background: capturing ? 'var(--muted)' : '#111110', color: 'white', border: 'none', borderRadius: 10, cursor: capturing ? 'default' : 'pointer', fontFamily: 'var(--sans)', fontSize: 13, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M4 12v8a2 2 0 002 2h12a2 2 0 002-2v-8"/><polyline points="16 6 12 2 8 6"/><line x1="12" y1="2" x2="12" y2="15"/>
                  </svg>
                  {capturing ? 'Working...' : 'Share'}
                </button>
              </div>
            </div>
          )}
