...
function MemoirIllustration({ seed, locations }: { seed: number; locations: string[] }) {
  ...
  const illustrations = [
    <svg key="0" viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: 'auto' }}>
      <defs><linearGradient id="sky0" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0a0a0a"/><stop offset="100%" stopColor="#1a2a1a"/></linearGradient><style>{`.m0s{animation:mTwinkle 2s ease-in-out infinite alternate}.m0w1{animation:mWave 4s ease-in-out infinite}.m0w2{animation:mWave 4s ease-in-out infinite 1.3s}.m0b{animation:mBird 3s ease-in-out infinite}@keyframes mTwinkle{0%{opacity:.2}100%{opacity:1}}@keyframes mWave{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}@keyframes mBird{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}}`}</style></defs>
      <rect width="400" height="200" fill="url(#sky0)"/>
      {[{x:40,y:20,r:1.5,d:.3},{x:80,y:35,r:1,d:.8},{x:130,y:15,r:2,d:.1},{x:200,y:28,r:1.5,d:.5},{x:260,y:12,r:1,d:1.2},{x:320,y:30,r:2,d:.7},{x:370,y:18,r:1.5,d:.4},{x:60,y:50,r:1,d:.9},{x:170,y:42,r:1.5,d:.2},{x:340,y:45,r:1,d:1.4}].map((s2,i)=>(
        <circle key={i} cx={s2.x} cy={s2.y} r={s2.r} fill={i%2===0?"#2e7d32":"white"} opacity=".7" className="m0s" style={{animationDelay:`${s2.d}s`}}/>
      ))}
      ...
      <path d="M180,50 L165,72 L195,72Z" fill="rgba(255,255,255,.15)"/><path d="M300,60 L288,78 L312,78Z" fill="rgba(255,255,255,.1)"/>
      ...
      <circle cx="50" cy="155" r="18" fill="none" stroke="#2e7d32" strokeWidth="1" opacity=".4"/>
      <path d="M50,140 L47,153 L50,150 L53,153Z" fill="#2e7d32" opacity=".6" className="m0b"/>
    ...
    <svg key="1" viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: 'auto' }}>
      ...
      {[{x:30,y:25,d:.1},{x:90,y:15,d:.5},{x:160,y:30,d:.9},{x:230,y:10,d:.3},{x:310,y:22,d:.7},{x:380,y:18,d:1.1}].map((st,i)=>(
        <circle key={i} cx={st.x} cy={st.y} r="1.5" fill={i%2===0?"#2e7d32":"white"} className="m1s" style={{animationDelay:`${st.d}s`}} opacity=".6"/>
      ))}
      <circle cx="200" cy="30" r="16" fill="#f5f0e8" opacity=".85"/><circle cx="208" cy="25" r="13" fill="#0d1f1a"/>
      <ellipse cx="200" cy="155" rx="4" ry="20" fill="rgba(245,240,232,.08)"/>
      ...
      <g className="m1b" style={{animationDelay:'1.2s'}}><path d="M200,50 Q209,43 218,50" fill="none" stroke="#2e7d32" strokeWidth="1.5" strokeLinecap="round" opacity=".6"/></g>
      <text x="200" y="192" fontFamily="Georgia,serif" fontSize="11" fill="rgba(255,255,255,.25)" textAnchor="middle" fontStyle="italic">{loc}</text>
    </svg>,
    <svg key="2" viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: 'auto' }}>
      <defs><linearGradient id="dusk2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#0a0a12"/><stop offset="100%" stopColor="#0a0a0a"/></linearGradient><style>{`.m2w{animation:m2Win 3s ease-in-out infinite alternate}.m2s{animation:m2Star 2s infinite alternate}@keyframes m2Win{0%{opacity:.15}100%{opacity:.55}}@keyframes m2Star{0%{opacity:.1}100%{opacity:.8}}`}</style></defs>
      ...
      <rect x="50" y="110" width="40" height="90" fill="#0f0f0f"/><rect x="55" y="90" width="30" height="25" fill="#0f0f0f"/>
      <rect x="100" y="85" width="25" height="115" fill="#0a0a0a"/><rect x="135" y="105" width="35" height="95" fill="#0f0f0f"/>
      <rect x="180" y="70" width="20" height="130" fill="#0a0a0a"/><rect x="210" y="90" width="45" height="110" fill="#0f0f0f"/>
      <rect x="265" y="100" width="30" height="100" fill="#0a0a0a"/><rect x="305" y="85" width="40" height="115" fill="#0f0f0f"/>
      <rect x="355" y="95" width="25" height="105" fill="#0a0a0a"/><rect x="375" y="80" width="25" height="120" fill="#0f0f0f"/>
      {[{x:18,y:105,d:.2},{x:58,y:98,d:.7},{x:58,y:115,d:1.2},{x:102,y:92,d:.4},{x:183,y:78,d:.9},{x:215,y:97,d:.1},{x:235,y:97,d:1.5},{x:270,y:108,d:.6},{x:308,y:92,d:1.1},{x:358,y:102,d:.3}].map((w,i)=>(
        <rect key={i} x={w.x} y={w.y} width="5" height="4" fill="#2e7d32" className="m2w" style={{animationDelay:`${w.d}s`}} rx=".5" opacity=".4"/>
      ))}
      {[{x:40,y:20},{x:150,y:25},{x:290,y:18}].map((st,i)=>(<circle key={i} cx={st.x} cy={st.y} r="1.2" fill="white" className="m2s" style={{animationDelay:`${i*.4}s`}} opacity=".4"/>))}
    ...
    <svg key="3" viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: 'auto' }}>
      <defs><linearGradient id="forest3" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#060d06"/><stop offset="100%" stopColor="#0a160a"/></linearGradient><style>{`.m3g{animation:m3Glow 2s ease-in-out infinite}.m3l{animation:m3Leaf 4s ease-in-out infinite}.m3b{animation:m3Bird 3s ease-in-out infinite}@keyframes m3Glow{0%,100%{opacity:.05}50%{opacity:.5}}@keyframes m3Leaf{0%,100%{transform:rotate(-5deg)}50%{transform:rotate(5deg) translateX(4px)}}@keyframes m3Bird{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}}`}</style></defs>
      <rect width="400" height="200" fill="url(#forest3)"/>
      <ellipse cx="200" cy="80" rx="60" ry="40" fill="#2e7d32" opacity=".05"/>
      <path d="M0,200 L30,80 L60,200Z" fill="#0a140a"/><path d="M20,200 L55,60 L90,200Z" fill="#0d180d"/>
      <path d="M50,200 L80,90 L110,200Z" fill="#0a140a"/><path d="M400,200 L370,80 L340,200Z" fill="#0a140a"/>
      ...
      {[{x:100,y:130},{x:130,y:110},{x:270,y:120},{x:300,y:140},{x:80,y:150}].map((f,i)=>(<circle key={i} cx={f.x} cy={f.y} r="2.5" fill="#2e7d32" className="m3g" style={{animationDelay:`${i*.5}s`}} opacity=".6"/>))}
      <g className="m3l" style={{transformOrigin:'120px 90px'}}><ellipse cx="120" cy="90" rx="8" ry="4" fill="#2e7d32" opacity=".4" transform="rotate(-30,120,90)"/></g>
      <g className="m3l" style={{transformOrigin:'280px 110px',animationDelay:'1s'}}><ellipse cx="280" cy="110" rx="6" ry="3" fill="#2e7d32" opacity=".35" transform="rotate(20,280,110)"/></g>
      <text x="200" y="193" fontFamily="Georgia,serif" fontSize="11" fill="rgba(255,255,255,.2)" textAnchor="middle" fontStyle="italic">{loc}</text>
    </svg>,
    <svg key="4" viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: 'auto' }}>
      ...
      <path d="M50,0 Q200,60 350,0" fill="none" stroke="rgba(255,255,255,.03)" strokeWidth="35"/>
      {Array.from({length:30}).map((_,i)=>({x:Math.abs(Math.sin(i*137)*380),y:Math.max(5,Math.abs(Math.cos(i*97)*75)+5)})).map((st,i)=>(
        <circle key={i} cx={st.x} cy={st.y} r={i%7===0?2:i%3===0?1.5:1} fill={i%3===0?"#2e7d32":"white"} className="m4s" style={{animationDelay:`${(i*.25)%3}s`}} opacity=".4"/>
      ))}
      ...
      <g className="m4c" style={{transformOrigin:'70px 160px'}}><rect x="67" y="125" width="6" height="35" fill="#0d1a0d" rx="3"/><rect x="55" y="138" width="14" height="5" fill="#0d1a0d" rx="2"/><rect x="69" y="132" width="5" height="14" fill="#0d1a0d" rx="2"/></g>
      <g className="m4c" style={{transformOrigin:'330px 160px',animationDelay:'.5s'}}><rect x="327" y="130" width="5" height="30" fill="#0d1a0d" rx="2"/><rect x="318" y="142" width="11" height="4" fill="#0d1a0d" rx="2"/></g>
      <circle cx="200" cy="162" r="12" fill="none" stroke="#2e7d32" strokeWidth=".8" opacity=".25"/>
    ...
    <svg key="5" viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: 'auto' }}>
      ...
      {[{x:50,y:20},{x:90,y:8},{x:140,y:25},{x:200,y:12},{x:260,y:20},{x:310,y:15},{x:360,y:25},{x:70,y:45},{x:170,y:38},{x:330,y:40}].map((st,i)=>(<circle key={i} cx={st.x} cy={st.y} r={i%3===0?1.8:1} fill={i%2===0?"#2e7d32":"white"} className="m5s" style={{animationDelay:`${i*.2}s`}} opacity=".5"/>))}
      ...
      {[0,50,100,150,200,250,300,350,400].map((x,i)=>(<line key={i} x1={x} y1="158" x2={x} y2="185" stroke="#141422" strokeWidth="1.5"/>))}
      <g className="m5l" style={{transformOrigin:'150px 155px'}}><ellipse cx="150" cy="155" rx="8" ry="10" fill="#2e7d32" opacity=".15"/><circle cx="150" cy="150" r="4" fill="#2e7d32" opacity=".3"/></g>
      ...
      <ellipse cx="200" cy="175" rx="35" ry="8" fill="#0f0f20"/><line x1="200" y1="175" x2="200" y2="200" stroke="#0f0f20" strokeWidth="4"/>
...
// Post vs Story format dimensions
...
export default function MemoirView({ entries, showToast }: MemoirViewProps) {
  ...
  const generateMemoir = async () => {
    ...
    const allRawWords = entriesToUse.map(e => e.text).join(' ');
    const wordCount = allRawWords.split(' ').length;

    try {
      const text = await callClaude({
        model: 'llama-3.3-70b-versatile',
        max_tokens: 900,
        system: `You are a poet and travel diarist. Your job is to take someone's raw travel journal entries and weave them into a richly textured, earthen, sensory travel diary piece — the kind that makes a content creator's followers stop scrolling.

CORE RULE: Use the traveller's ACTUAL words as much as possible. Lift their exact phrases. Preserve their specific details — the specific smell, the specific name, the specific moment. Your job is not to replace their voice but to elevate it. If they wrote "cold limestone" keep "cold limestone". If they wrote "the man raking gravel" keep that man. You are their ghost-writer, not their replacement.

TONE: Earthen, nature-connected, slow. Think old ink on paper. Campfire warmth. Soil after rain. The kind of writing that makes you want to go outside. Grounded. Human. Never sterile.

STRUCTURE for content creators:
- Open with ONE killer sensory line that drops the reader into the place
- Weave the locations and moments together as one continuous experience
- Use short punchy paragraphs (2-3 sentences) alternating with one long flowing sentence
- Include 2-3 lines that would work perfectly as standalone Instagram captions
- End with a one-line truth about what travel does to a person — philosophical but plain-spoken

LENGTH: ${wordCount > 200 ? '280-350' : '200-260'} words.

BANNED: vibrant, bustling, hidden gem, tapestry, mosaic, wanderlust, breathtaking, stunning, picturesque, charming, quaint, off the beaten path, journey of self-discovery.

FORMAT: plain prose, no headers, no bullet points, no hashtags.`,
        messages: [{ role: 'user', content: `My travel journal entries:\n\n${entrySummary}\n\nWrite my travel diary memoir. Use my words. Make it shareable.` }],
      });
  ...
  // Post format: landscape card, full memoir
  // Story format: portrait card, first 3 lines + "read more"
  const storyText = memoir.split('\n').filter(l => l.trim()).slice(0, 3).join('\n\n');
  const displayText = shareFormat === 'story' ? storyText : memoir;

  const shareTextFull = `${memoir}\n\n📍 ${allLocations}\n\n— from my Safarnama सफ़रनामा\n#traveldiary #slowtravel #travelwriting`;
  const shareTextStory = `${storyText}\n\n📍 ${primaryLocation}\n\n— Safarnama सफ़रनामा`;
    ...
    <div>
      ...
      <div style={{ marginBottom: 40, textAlign: 'center', padding: '40px 0 0', position: 'relative' }}>
        ...
        <div style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--muted)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
          {entries.length} {entries.length === 1 ? 'moment' : 'moments'} · earthen · poetic · shareable
        </div>
      </div>
      ...
      {entries.length > 1 && (
        <div style={{ marginBottom: 24 }}>
          ...
          {selectMode && selectedIds.size > 0 && (<div style={{ marginTop: 8, fontFamily: 'var(--sans)', fontSize: 11, color: '#2e7d32', fontWeight: 500 }}>{selectedIds.size} entries selected</div>)}
        </div>
      )}
          ...
          {entries.map((entry) => {
            ...
            return (
              <button key={entry.id} onClick={() => toggleEntry(entry.id)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', border: `1.5px solid ${selected ? '#2e7d32' : 'var(--border)'}`, background: selected ? '#f0f7f0' : 'var(--surface)', borderRadius: 10, cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s', width: '100%' }}>
                <div style={{ width: 20, height: 20, borderRadius: '50%', border: `2px solid ${selected ? '#2e7d32' : '#ccc'}`, background: selected ? '#2e7d32' : 'transparent', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s' }}>
      ...
      {/* Generate CTA */}
      <button onClick={generateMemoir} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '22px 28px', background: 'linear-gradient(135deg,#111110 0%,#1a2a1a 100%)', border: 'none', borderRadius: 12, cursor: 'pointer', transition: 'all 0.2s', marginBottom: 4, boxShadow: '0 4px 24px rgba(46,125,50,.12)' }}>
        <div style={{ textAlign: 'left' }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, fontStyle: 'italic', color: '#fff', marginBottom: 4 }}>{memoir ? 'Write a new version' : 'Generate my travel diary'}</div>
          <div style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'rgba(255,255,255,.4)', fontWeight: 300 }}>earthy · poetic · ready to share</div>
        </div>
          ...
          {memoirLoading ? (
            ...
            <div>
              ...
              <div style={{ background: '#111110', borderRadius: '0 0 16px 16px', padding: '28px 28px 24px', borderTop: '3px solid #2e7d32' }}>
                ...
                <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 16, fontWeight: 400, lineHeight: 2, color: 'rgba(255,255,255,.85)', whiteSpace: 'pre-wrap', marginBottom: 24 }}>{memoir}</div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,.08)', paddingTop: 16 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => { navigator.clipboard.writeText(shareTextFull); showToast('Copied!'); }} style={{ fontFamily: 'var(--sans)', fontSize: 11, fontWeight: 500, background: 'rgba(255,255,255,.08)', color: 'rgba(255,255,255,.6)', border: '1px solid rgba(255,255,255,.12)', padding: '7px 14px', borderRadius: 6, cursor: 'pointer' }}>Copy</button>
                    <button onClick={() => setShowShareCard(!showShareCard)} style={{ fontFamily: 'var(--sans)', fontSize: 11, fontWeight: 500, background: '#2e7d32', color: 'white', border: 'none', padding: '7px 14px', borderRadius: 6, cursor: 'pointer' }}>Share ↗</button>
                  </div>
                </div>
              </div>
              ...
              {/* Share panel */}
              {showShareCard && (
                <div style={{ marginTop: 12, background: 'var(--surface)', borderRadius: 16, overflow: 'hidden', animation: 'fadeUp 0.25s ease' }}>
                  {/* Post / Story toggle */}
                  <div style={{ display: 'flex', padding: '16px 20px 0', gap: 8, borderBottom: '1px solid var(--border)' }}>
                    {(['post', 'story'] as ShareFormat[]).map(fmt => (
                      <button key={fmt} onClick={() => setShareFormat(fmt)} style={{ fontFamily: 'var(--sans)', fontSize: 12, fontWeight: shareFormat === fmt ? 600 : 400, color: shareFormat === fmt ? 'var(--text)' : 'var(--muted)', background: 'none', border: 'none', padding: '8px 16px', cursor: 'pointer', borderBottom: `2px solid ${shareFormat === fmt ? 'var(--text)' : 'transparent'}`, marginBottom: -1, transition: 'all 0.15s', textTransform: 'capitalize' }}>
                        {fmt === 'post' ? '▭ Post' : '▯ Story'}
                      </button>
                    ))}
                    <div style={{ marginLeft: 'auto', fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--muted)', alignSelf: 'center', paddingRight: 4 }}>
                      {shareFormat === 'post' ? 'full memoir · feed format' : 'first 3 lines · vertical format'}
                    </div>
                  </div>

                  {/* Preview */}
                  <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
                    <div style={{ fontFamily: 'var(--sans)', fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 10 }}>Preview</div>
                    <div style={{
                      background: '#111110',
                      borderRadius: 10,
                      padding: '16px',
                      maxHeight: shareFormat === 'story' ? 200 : 160,
                      overflow: 'hidden',
                      position: 'relative',
                      border: '1px solid rgba(255,255,255,.06)'
                    }}>
                      <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 13, lineHeight: 1.8, color: 'rgba(255,255,255,.8)', whiteSpace: 'pre-wrap' }}>{displayText}</div>
                      {shareFormat === 'post' && memoir.length > 400 && (
                        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 40, background: 'linear-gradient(transparent, #111110)' }}/>
                      )}
                    </div>
                    <div style={{ marginTop: 8, fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--muted)' }}>
                      📍 {shareFormat === 'story' ? primaryLocation : allLocations}
                    </div>
                  </div>

                  {/* Share buttons */}
                  <div style={{ padding: '16px 20px 20px' }}>
                    <div style={{ fontFamily: 'var(--sans)', fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 12 }}>Share as {shareFormat}</div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                      <a href={`https://wa.me/?text=${encodeURIComponent(shareFormat === 'post' ? shareTextFull : shareTextStory)}`} target="_blank" rel="noopener noreferrer"
                      ...
                      <a href="https://www.instagram.com/" target="_blank" rel="noopener noreferrer"
                        onClick={() => { navigator.clipboard.writeText(shareFormat === 'post' ? shareTextFull : shareTextStory); showToast(`${shareFormat === 'post' ? 'Post' : 'Story'} caption copied — paste in Instagram`); }}
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px', background: 'linear-gradient(45deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)', color: 'white', borderRadius: 10, textDecoration: 'none', fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 500 }}>
                        ...
                        Instagram {shareFormat === 'story' ? 'Story' : 'Post'}
                      </a>
                      <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent((shareFormat === 'post' ? memoir.slice(0,240) : storyText.slice(0,240))+'...\n\n📍 '+primaryLocation+'\n\n— Safarnama सफ़रनामा #traveldiary #slowtravel')}`}
                        target="_blank" rel="noopener noreferrer"
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px', background: '#000', color: 'white', borderRadius: 10, textDecoration: 'none', fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 500 }}>
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="white"><path d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"/></svg>
                        X / Twitter
                      </a>
                      <button onClick={() => { navigator.clipboard.writeText(shareFormat === 'post' ? shareTextFull : shareTextStory); showToast('Copied!'); }}
                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px', background: 'var(--bg)', color: 'var(--text)', border: '1px solid var(--border)', borderRadius: 10, cursor: 'pointer', fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 500 }}>
                        Copy text
                      </button>
                    </div>
                    <button onClick={generateMemoir} style={{ width: '100%', marginTop: 10, padding: '10px', background: 'transparent', color: 'var(--muted)', border: '1px solid var(--border)', borderRadius: 8, cursor: 'pointer', fontFamily: 'var(--sans)', fontSize: 12 }}>
                      ↺ Regenerate
                    </button>
                  </div>
                </div>
