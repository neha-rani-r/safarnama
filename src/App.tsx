...
const STYLES = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .memoir-section { margin-top: 40px; }
  .memoir-trigger {
    width: 100%;
    display: flex;
    align-items: center;
  ...
  .modal-action-btn.primary { background: var(--text); color: var(--bg); }
  .modal-action-btn.secondary { background: var(--surface); color: var(--text); border: 1px solid var(--border); }
  .modal-action-btn:hover { opacity: 0.8; }
  .modal-close {
    position: absolute;
    top: 16px; right: 16px;
    background: none;
...
export default function App() {
  ...
  // Collect unique emotions present in entries for filter chips
    ...
    <>
      ...
      <div className="app">
        ...
        <nav className="tabs">
          ...
          <button
            className={`tab ${tab === 'memoir' ? 'active' : ''}`}
            ...
            {tab === 'write' && (
              <div className="composer">
                ...
                <div className="active-prompt">
                  ...
                  <div className="prompt-why">
                    {PROMPTS[selectedPrompt].why.toUpperCase()}
                  </div>
                </div>

                {/* Compass watermark — faint, centered, CSS-only needle rock */}
